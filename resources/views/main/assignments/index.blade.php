<x-app-layout>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        @forelse($assignments ?? [] as $assignment)
            <div class="bg-[#EBEBFF] rounded-2xl p-6 relative hover:shadow-md transition-shadow group flex flex-col h-full border border-indigo-50">
                ...
                <div class="absolute top-4 right-4 flex gap-2 z-10">
                    ...
                    <form action="{{ route('assignments.destroy', $assignment->id) }}" method="POST" onsubmit="return confirm('Delete this assignment?');">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="p-2 bg-[#ffdede] text-red-500 rounded-lg hover:bg-red-500 hover:text-white transition-colors shadow-sm" title="Delete">
                ...
                <a href="{{ route('assignments.show', $assignment->id) }}" class="hover:underline decoration-[#764BA2] decoration-2 underline-offset-2">
                    <h3 class="text-lg font-bold text-gray-900 mb-2 pr-24 leading-tight">
                        {{ $assignment->title }}
                    </h3>
                </a>
                ...
                <div class="pt-4 border-t border-indigo-200/50 flex flex-col gap-1 text-xs font-medium text-gray-500">
                    <div class="flex items-center justify-between">
                        ...
                        <span class="text-[#764BA2] font-bold bg-indigo-50 px-2 py-0.5 rounded-md border border-indigo-100">
