<x-create-layout title="Create New Assignment" description="Set up a new task and link it to a course." backRoute="{{ route('assignments.index') }}" maxWidth="5xl">
    <form method="POST" action="{{ route('assignments.store') }}" class="p-6 sm:p-8 space-y-8">
        ...
        <div class="space-y-2">
            ...
            <textarea name="description" rows="5" placeholder="Write the details and requirements of the assignment here..."
                class="w-full px-4 py-3 rounded-xl border-slate-200 text-slate-700 font-medium focus:border-[#764BA2] focus:ring focus:ring-[#764BA2]/10 transition-all bg-white resize-none" required></textarea>
            @error('description')
                <p class="text-xs text-red-500 mt-1 ml-1">{{ $message }}</p>
            @enderror
        </div>
