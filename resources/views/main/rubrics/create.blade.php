<x-create-layout title="Create New Rubric" description="Define criteria with performance levels for structured grading."
    ...
    <form method="POST" action="{{ route('rubrics.store') }}" x-data="rubricHandler()" class="p-6 sm:p-8 space-y-8">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div id="assignment_dropdown" class="absolute z-10 w-full mt-1 bg-white border border-slate-200 rounded-xl shadow-lg max-h-60 overflow-y-auto hidden">
                        ...
                        <div id="assignment_options">
                            @foreach ($assignments as $assignment)
                                <div class="assignment-option px-4 py-3 hover:bg-slate-50 cursor-pointer text-slate-700 font-medium" data-value="{{ $assignment->id }}" data-text="{{ $assignment->title }} ({{ $assignment->course->name ?? 'No Course' }})">
            ...
            <div class="space-y-2">
                ...
                <input type="text" name="subject_name" placeholder="e.g. Cloud Computing Fundamentals"
        ...
        <div class="space-y-4">
            ...
            <template x-for="(criterion, cIndex) in criteria" :key="criterion.id">
        ...
        function rubricHandler() {
            return {
                criteria: [{
                    ...
                    weight: 0,
                    levels: [
                        { min: 0, max: 44.99, grade: 'E', description: '' }
                    ]
                }],
